...
Credible Layer:
      --rpc.credible-registry-address <ADDRESS>
          Address of the on-chain `CredibleRegistry` contract.

          When set, the marker override for `eth_call` / `eth_estimateGas` is derived per-request from the registry's `_credibleBlocks` mapping instead of a static override.
