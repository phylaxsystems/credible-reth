...
Credible Layer:
      --rpc.credible-registry-address <ADDRESS>
          ...
          When set, the marker override for call-like RPC methods is derived per-request from the registry's `_credibleBlocks` mapping instead of a static override.
